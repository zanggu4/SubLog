...
export default function Home() {
  ...
  const handleLanguageChange = (newLang: Language) => {
    setLanguage(newLang);
    const newPath = pathname.replace(/^\/(en|ko|ja|zh)/, `/${newLang}`);
    router.replace(newPath);
  };

  const seo = translations[language]?.seo ?? translations.ko.seo;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-center items-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-4 right-4 flex items-center gap-2 z-50">
        <div className="relative flex items-center gap-1 text-muted bg-card px-2 py-1.5 rounded-md text-xs border border-border cursor-pointer">
